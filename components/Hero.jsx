...
//         className="w-full lg:w-1/2 z-10 space-y-6 sm:space-y-8 text-center lg:text-left mb-8 lg:mb-0"
//       >
  ...
  useEffect(() => {
    ...
    if (isMobile) {
      ...
      const animate = () => {
        ...
        container.style.transform = `translateX(-${Math.abs(scrollPosition)}px)`;
        animationRef.current = requestAnimationFrame(animate);
      };
...
export default function DiagonalHeroSection() {
  ...
  return (
    <section className={`relative w-full rounded-t-2xl bg-gradient-to-br from-orange-50 via-amber-50 to-yellow-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-950 mt-10 mx-auto px-4 overflow-hidden text-gray-900 dark:text-white ${
      ...
      {/* Mobile Layout */}
      <div className={`${isMobile ? 'flex' : 'hidden'} flex-col items-center justify-center text-center space-y-8`}>
        <motion.div
